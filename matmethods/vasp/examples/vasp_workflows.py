...
def get_wf_elastic_Vasp(structure, vasp_input_set=None, vasp_cmd="vasp",
                        db_file=None, custodian_powerup=False):
    """
    Return vasp workflow consisting of 25 fireworks needed to fit the elastic
    tensor for a given structure

    Firework 1 : write vasp input set for structural relaxation, run vasp,
        pass run location and database insertion.
    Fireworks 2-25 : construct deformed structure, write vasp input set for 
    ionic relaxation, run vasp, database insertion, elastic tensor fitting
    ...
    task_label = "elastic setup"
    t2 = []
    t2.append(SetupElasticTask())
    t2.append(WriteVaspStaticFromPrev())
    t2.append(RunVaspDirect(vasp_cmd=vasp_cmd))
    t2.append(PassVaspLocs(name=task_label))
    t2.append(VaspToDbTask(db_file=db_file,
