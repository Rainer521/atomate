...
def get_wf_elastic_constant(structure, norm_deformations=[0.01], shear_deformations=[0.03], 
    ...
    Args:
        ...
        kwargs (keyword arguments): additional kwargs to be passed to get_wf_deformations
            e. g. vasp_cmd, user_kpoints_density, optimize_structure
    ...
    strains = []
    for nd in norm_deformations:
        strains.extend(np.eye(6)[:3]*nd)
    for sd in shear_deformations:
        strains.extend(np.eye(6)[3:]*sd)

    defos = [Strain.from_voigt(strain).deformation_matrix for strain in strains]
    wf_elastic = get_wf_deformations(structure, defos, pass_stress_strain=True, 
...
def get_wf_elastic_toec(structure, max_strain=0.05, stencil_res=5, indices=None, **kwargs):
    """
    ...
    if stencil_res % 2 != 1 or stencil_res < 5:
        raise ValueError("Stencil resolution must be an odd integer greater than 5")
